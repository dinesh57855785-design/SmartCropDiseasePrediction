...
class ApiService {
  ...
  static Future<void> configureBaseUrl({String? url}) => ApiConfig.configure(url: url);

  static Future<http.Response> get(String path, {Map<String, String>? query}) async {
    final uri = Uri.parse('${ApiConfig.effectiveBaseUrl}$path').replace(queryParameters: query);
  ...
  static Future<http.Response> postMultipart(String path, Map<String, String> fields, {File? imageFile}) async {
    final uri = Uri.parse('${ApiConfig.effectiveBaseUrl}$path');
    final token = await getToken();
